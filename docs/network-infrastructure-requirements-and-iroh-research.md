...
虚拟地址分为服务端地址与客户端地址。`AppAddr` 标识一个应用身份：服务端直接绑定其服务应用地址，客户端以“客户端应用地址 + DeviceId”标识调用方。来源客户端的 `AppAddr` 与目标服务端的 `AppAddr` 相互独立，不要求相等；同一个客户端身份可以连接网络内多个不同的服务应用。

以下 Rust 类型用于表达语义，32 字节长度是建议值，最终编码可在协议设计阶段确定：

```rust
/// 由应用指定的稳定身份，不包含 IP、端口或 relay 信息。
...
- `AppAddr` 由应用配置或创建，表示该虚拟网络内稳定的应用身份；作为 `Server` 时是服务目标，作为 `Client` 的一部分时是调用方应用身份。默认由应用根公钥派生，所有权可以离线验证；人类指定的名称只作为别名。节点更换 IP、端口、relay 或承载机器时，该地址不变。
...
- 客户端拨号时其来源 `Client.app` 不得被目标地址覆盖；目标 `Server.app` 独立解析和验证。默认同一虚拟网络内任意有效 Client binding 都可连接任意有效 Server binding，显式服务 ACL 可以进一步收紧权限。
...
| 发起方 | 目标地址 | 语义 |
|---|---|---|
| 客户端 | `Server { app }` | 以自身 `Client { source_app, device }` 身份连接任意目标应用当前有效的服务端实例；来源与目标 app 可以不同。 |
...
- **ID-8 MUST**：来源客户端 `AppAddr` 与目标服务端 `AppAddr` 必须独立验证且不要求相等；一个有效客户端可以连接同一虚拟网络内多个服务端应用，除非显式 ACL 拒绝。
